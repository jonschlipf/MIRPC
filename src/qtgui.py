...
class SpectrumThreadI(QThread):
    ...
    def __init__(self,caller,ylabel,mono,lockin):
    ...
    def run(self):
        ...
        for i in range(lam.size):
            print("set wl")
            print(lam[i])
            self.mono.set_wavelength(lam[i])
            print("wl set")
...
class App(QWidget):
    ...
    def measure_diode(self):
        ...
        self.mono.set_wavelength(float(self.scan_lmin_box.text()))
        time.sleep(20*self.lockin.get_timeconst())
        self.thread=SpectrumThreadI(self,"I/A",self.mono,self.lockin)
    ...
    def measure_ref(self):
        self.lockin.set_input(0)
        self.mono.set_wavelength(float(self.scan_lmin_box.text()))
        time.sleep(20*self.lockin.get_timeconst())
        self.thread=SpectrumThreadV(self,"V/V",self.mono,self.lockin)
